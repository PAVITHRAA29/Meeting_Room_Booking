...
public class MeetingEntityAdminImplementation implements MeetingEntityAdminService {
    ...
    @Override
    @Cacheable(value = "meetingRequestsCache", key = "'request'")
...
//            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ResponseDTO(HttpStatus.OK.getReasonPhrase(), Strings.INTERNAL_ERROR, null));
        }
    }
    ...
    @Override
    @CacheEvict(value = "meetingRequestsCache", key = "'request'")
